...
from PyQt5.QtWidgets import (
    ...
    QCheckBox, QDialog, QProgressBar, QFormLayout,
)
...
def draw_aqh_schematic(ax, Nx, Ny, in_idx=None, out_idx=None, title="",
                          ...
                          site_grids=None, link_grids=None):
    ...
    # Precomputed per-ring perimeter intensities (e.g. the fast-time waveform
    # for the slow-time ring movie) take priority over the z-grid field.
    have_grids = site_grids is not None and link_grids is not None
    if have_grids:
        if I_max is None:
            all_vals = np.concatenate(list(site_grids.values())
                                        + list(link_grids.values()))
            I_max = max(float(np.max(all_vals)), 1e-30)
    elif template is not None and E is not None:
        site_idx = template["site_idx"]
    ...
    if have_grids or (template is not None and E is not None):
...
def plot_field_distribution(ax, E, Nx, Ny, template, title="", I_max=None,
                            site_grids=None, link_grids=None):
    """Draw the rounded-square lattice coloured by |E|^2 on each ring's perimeter.
    site_grids / link_grids may be supplied precomputed (one array of perimeter
    intensities per ring) to colour the rings by something other than the z-grid
    field -- e.g. the fast-time waveform (see plot_waveform_distribution)."""
    site_idx = template["site_idx"]
    link_idx = template["link_idx"]
...
def plot_waveform_distribution(ax, wf_by_grid, Nx, Ny, template, title="", I_max=None):
    """Like plot_field_distribution, but colour each ring's perimeter by its fast-time
    waveform |E(tau)|^2 (mapped onto the perimeter by cavity position z = tau), so a
    temporal pattern -- Turing rolls, a soliton -- shows as bright arcs on the ring.
    wf_by_grid maps a representative grid index (slot 0 of each ring) to its waveform."""
    site_idx = template["site_idx"]; link_idx = template["link_idx"]
    h_link_names = template["h_link_names"]; v_link_names = template["v_link_names"]
    site_grids = {(ix, iy): wf_by_grid[site_idx(ix, iy, 0)]
                  for iy in range(Ny) for ix in range(Nx)}
    link_grids = {name: wf_by_grid[link_idx(name, 0)]
                  for name in h_link_names + v_link_names}
    return plot_field_distribution(ax, None, Nx, Ny, template, title=title, I_max=I_max,
                                   site_grids=site_grids, link_grids=link_grids)


def plot_waveform_distribution_aqh(ax, wf_by_grid, Nx, Ny, template, title="", I_max=None):
    """AQH analogue of plot_waveform_distribution: colour each brick-wall ring's perimeter
    by its fast-time waveform |E(tau)|^2 (mapped onto the perimeter by cavity position
    z = tau). wf_by_grid maps the slot-0 grid index of each ring to its waveform, with
    site rings first then link rings (the order rep_grids is built in)."""
    site_idx = template["site_idx"]; link_idx = template["link_idx"]
    n_sites = len(aqh_site_positions(Nx, Ny))
    n_links = len(aqh_link_positions(Nx, Ny))
    site_grids = {i: wf_by_grid[site_idx(i, 0)] for i in range(1, n_sites + 1)}
    link_grids = {j: wf_by_grid[link_idx(j, 0)] for j in range(1, n_links + 1)}
    return draw_aqh_schematic(ax, Nx, Ny, template=template, title=title, I_max=I_max,
                              site_grids=site_grids, link_grids=link_grids)
...
def lattice_kerr_sweep(deltas, template, kappa_ex, alpha, beta0_eta_over_pi,
                        D2, gamma, pump_amp, n_modes, n_roundtrips, n_site_pts,
                        s_disp=-1.0, on_step=None, kerr_lumped=False, avg_frac=0.3,
                        seed_noise=1e-4, rng_seed=0, E0=None):
    """Faithful coupled-ring dispersive IKEDA MAP with Kerr, driving the SAME
    z-resolved propagator R as the linear time_evolve — site rings AND link rings
    are propagated explicitly (no effective Hamiltonian, nothing hidden).

    The comb is a fast-time waveform carried on every grid point: the field is
    E[grid, mu] with n_modes comb lines. One R-application advances every grid
    point by one Delta-z around its ring; Nz_site applications = one site round
    trip. R(omega) factorizes per mode -- every entry is coeff*exp(i*omega*dz)*
    (loss & structural phase), and the only omega-dependence is exp(i*omega*dz_site)
    (site edges) or exp(i*omega*dz_link) (link edges). So for comb mode mu at
    omega_mu = delta + s*D2*mu^2 (detuning + 2nd-order dispersion):

        E <- ms(mu) * (Csite @ E)  +  ml(mu) * (Clink @ E)      # linear, all modes
        E[src, 0] += pump_amp * src_vals                        # CW pump at mu = 0
        Kerr: E(tau) *= exp(i*gamma*|E(tau)|^2 * dz)  # ALL ring waveguides (site + link)

    The Kerr is distributed across the round trip by 2nd-order Strang split-step
    (kerr_lumped=False, default): a half-Kerr, then Nz pairs of [linear step,
    full-Kerr] with the last Kerr a half-step, so the whole sweep is one continuous
    split-step integration. kerr_lumped=True instead applies one Kerr kick per round
    trip (faster, valid when the per-round-trip nonlinear phase is small).

    with Csite/Clink the omega-independent coupling matrices (split by edge kind),
    ms/ml the per-mode site/link propagation phases. At gamma = 0 only mu = 0 is
    driven and this reduces EXACTLY to time_evolve -> the low-power drop curve is
    the linear drop spectrum (the band). Comb is read at the drop bus
    s_drop(tau) = i*kappa*p_site*E[drop](tau).

    Returns (comb_power[len(deltas)], final s_drop[n_modes]).
    """
    from scipy.sparse import csc_matrix
    Nz_site = template["Nz_site"]; Nz_link = template["Nz_link"]
    dz_site = 1.0 / Nz_site; dz_link = 1.0 / Nz_link
    ss = template["state_size"]
    rows = template["rows"]; cols = template["cols"]; kinds = template["kinds"]
    coeffs = template["coeffs"]; extras = template["extras_arr"]
    beta0_eta = beta0_eta_over_pi * np.pi
    # omega-INDEPENDENT parts of R (loss + structural/Peierls phases), by edge kind
    cval = np.where(kinds == 0,
                    -coeffs * np.exp(-alpha * dz_site / 2.0),
                    -coeffs * np.exp(1j * beta0_eta / Nz_link
                                     - alpha * dz_link / 2.0 + 1j * extras))
    sm = (kinds == 0)
    Csite = csc_matrix((cval[sm],  (rows[sm],  cols[sm])),  shape=(ss, ss))
    Clink = csc_matrix((cval[~sm], (rows[~sm], cols[~sm])), shape=(ss, ss))
    src_rows = template["src_rows_arr"]; src_vals = template["src_vals_arr"]
    # drop-bus grid point + its predecessor propagation factor (as in time_evolve).
    # IQH: bus_drop = (ix, iy, slot), site_idx(ix, iy, k). AQH: bus_drop_idx + slot,
    # site_idx(i, k).
    dir_sign = -1 if template.get("dc_flip", False) else +1
    site_idx = template["site_idx"]
    if "bus_drop" in template:
        i_d, j_d, s_d_slot = template["bus_drop"]
        pred_d = (s_d_slot - dir_sign) % Nz_site
        drop_grid = site_idx(i_d, j_d, pred_d)
    else:
        pred_d = (template["bus_drop_slot"] - dir_sign) % Nz_site
        drop_grid = site_idx(template["bus_drop_idx"], pred_d)

    N = int(n_modes)
    mu = np.fft.fftfreq(N, d=1.0 / N)            # comb mode numbers
    disp = s_disp * D2 * mu ** 2                  # 2nd-order dispersion per round trip
    nrt = int(n_roundtrips)
    navg = max(1, int(round(avg_frac * nrt)))

    def kerr(F, k):                              # Kerr kick in ALL rings (mode amps <-> field)
        et = np.fft.fft(F, axis=1)
        et *= np.exp(1j * gamma * (et.real ** 2 + et.imag ** 2) * k)
        return np.fft.ifft(et, axis=1)

    comb = np.empty(len(deltas))
    s_drop = np.zeros(N, dtype=complex)
    E = np.array(E0, dtype=complex) if E0 is not None else np.zeros((ss, N), dtype=complex)
    rng = np.random.default_rng(rng_seed)
    for i, delta in enumerate(deltas):
        if seed_noise > 0.0 and i == 0 and E0 is None:   # MI seed once (skip if a field is supplied);
            E[:n_site_pts] += seed_noise * (rng.standard_normal((n_site_pts, N))   # then the carried-
                                            + 1j * rng.standard_normal((n_site_pts, N)))  # forward field rides its branch
        wmu = delta + disp                        # omega_mu (carrier + dispersion), length N
        ms = np.exp(1j * wmu * dz_site)           # site propagation phase per mode
        ml = np.exp(1j * wmu * dz_link)           # link propagation phase per mode
        pdrop = np.exp(1j * wmu * dz_site - alpha * dz_site / 2.0)   # drop read factor
        acc = 0.0; cnt = 0
        for rt in range(nrt):
            if (not kerr_lumped) and gamma != 0.0:
                # distributed Strang split-step (2nd order): half-Kerr, then
                # [linear+pump, full-Kerr] x Nz with the last Kerr a half-step.
                # Consecutive half-Kerrs at round-trip boundaries merge into full
                # steps -> a single continuous split-step integration (Ekstrom 5.9).
                E = kerr(E, 0.5 * dz_site)
                for z in range(Nz_site):
                    E = ms[None, :] * (Csite @ E) + ml[None, :] * (Clink @ E)
                    E[src_rows, 0] += pump_amp * src_vals
                    E = kerr(E, dz_site if z < Nz_site - 1 else 0.5 * dz_site)
            else:
                # linear (gamma=0) or lumped Kerr (one kick per round trip, fast)
                for z in range(Nz_site):
                    E = ms[None, :] * (Csite @ E) + ml[None, :] * (Clink @ E)
                    E[src_rows, 0] += pump_amp * src_vals
                if gamma != 0.0:
            ...
            if not np.isfinite(E).all():
                E[:] = 0.0; break
            if rt >= nrt - navg:
                # sd is the drop-port COMB SPECTRUM (mode domain): sd[mu].
                sd = 1j * kappa_ex * pdrop * E[drop_grid]
                acc += float(np.sum(np.abs(sd) ** 2) - np.abs(sd[0]) ** 2)  # comb = all mu != 0
                cnt += 1
        s_drop = 1j * kappa_ex * pdrop * E[drop_grid]      # drop comb spectrum, mode domain
        comb[i] = acc / max(cnt, 1)
        if on_step is not None:
            on_step(i, float(delta), comb[i], s_drop, E)
    return comb, s_drop


def slow_time_evolve(E0, template, kappa_ex, alpha, beta0_eta_over_pi, D2, gamma,
                      pump_amp, delta, n_roundtrips, s_disp=-1.0, kerr_lumped=False,
                      record_grid=None, rep_grids=None, on_rt=None):
    """Hold the detuning fixed at `delta` and evolve the settled field E0 for
    n_roundtrips, recording each round trip: (a) the drop-ring fast-time waveform
    |E(tau)|^2 (the 2D slow-time map), and (b) the waveform at each representative
    grid point in rep_grids (one per lattice ring, for the ring movie). Returns
    (wf[n_rt, N], wfr[n_rt, n_reps, N]). Uses the SAME Strang split-step machinery
    as lattice_kerr_sweep. on_rt(rt) is an optional progress callback.
    """
    from scipy.sparse import csc_matrix
    Nz_site = template["Nz_site"]; Nz_link = template["Nz_link"]
    dz_site = 1.0 / Nz_site; dz_link = 1.0 / Nz_link
    ...
    Clink = csc_matrix((cval[~sm], (rows[~sm], cols[~sm])), shape=(ss, ss))
    src_rows = template["src_rows_arr"]; src_vals = template["src_vals_arr"]
    if record_grid is None:                       # default: the drop-bus ring point
        dir_sign = -1 if template.get("dc_flip", False) else +1
        if "bus_drop" in template:                # IQH (ix, iy, slot)
            i_d, j_d, s_d_slot = template["bus_drop"]
            pred_d = (s_d_slot - dir_sign) % Nz_site
            record_grid = template["site_idx"](i_d, j_d, pred_d)
        else:                                     # AQH (site index, slot)
            pred_d = (template["bus_drop_slot"] - dir_sign) % Nz_site
            record_grid = template["site_idx"](template["bus_drop_idx"], pred_d)

    E = np.array(E0, dtype=complex)
    N = E.shape[1]
    mu = np.fft.fftfreq(N, d=1.0 / N)
    wmu = delta + s_disp * D2 * mu ** 2
    ms = np.exp(1j * wmu * dz_site); ml = np.exp(1j * wmu * dz_link)
    ...
    reps = (np.asarray(rep_grids, dtype=int) if rep_grids is not None
            else np.array([record_grid], dtype=int))
    nrt = int(n_roundtrips)
    wf = np.empty((nrt, N))                        # drop-ring waveform |E(tau)|^2 (2D map)
    wfr = np.empty((nrt, len(reps), N))            # per-ring waveform (lattice ring movie)
    for rt in range(nrt):
        if (not kerr_lumped) and gamma != 0.0:
            E = kerr(E, 0.5 * dz_site)
            for z in range(Nz_site):
        ...
        if not np.isfinite(E).all():
            E[:] = 0.0
        wf[rt] = np.fft.fftshift(np.abs(np.fft.fft(E[record_grid])) ** 2)  # drop |E(tau)|^2
        wfr[rt] = np.fft.fftshift(np.abs(np.fft.fft(E[reps], axis=1)) ** 2, axes=1)  # per ring
        if on_rt is not None:
            on_rt(rt)
    return wf, wfr


from matplotlib.ticker import Locator as _Locator, FuncFormatter as _FuncFormatter


class _EndsLocator(_Locator):
    """Major-tick locator: exactly two ticks, at the current axis min and max."""
    def __call__(self):
...
def _ends_ticks(ax, xaxis=True, yaxis=True):
    """Show only two ticks/labels on an axis: the minimum and the maximum."""
    f = _FuncFormatter(lambda v, _p: f'{v:.4g}')
    if xaxis:
        ax.xaxis.set_major_locator(_EndsLocator())
        ax.xaxis.set_major_formatter(_FuncFormatter(lambda v, _p: f'{v:.4g}'))
    if yaxis:
        ax.yaxis.set_major_locator(_EndsLocator())
        ax.yaxis.set_major_formatter(f)


class _SweepStopped(Exception):
    """Raised from on_step to abort a running detuning sweep (Stop button)."""


class SlowTimeDialog(QDialog):
    """Lattice movie of the slow-time dynamics on the rounded-square rings (the EXACT
    linear-app renderer), each ring coloured by its fast-time waveform |E(tau)|^2 mapped
    onto its perimeter (cavity position z = tau). A Turing pattern shows as bright arcs
    on the ring, a soliton as one bright arc; over round trips they breathe / drift /
    scramble. Slider + movie export."""
    def __init__(self, parent, wfr, rep_grids, Nx, Ny, template, is_aqh, delta_dialog):
        super().__init__(parent)
        self.setWindowTitle('Slow-time — ring movie')
        self.resize(640, 720)
        self.setStyleSheet(parent.styleSheet())
        self.wfr = wfr                       # (n_roundtrips, n_reps, N) per-ring waveform
        self.rep_grids = rep_grids           # grid index for each column of wfr
        self.nrt = wfr.shape[0]
        self.Imax = float(wfr.max()) if wfr.max() > 0 else 1.0
        self.Nx = Nx; self.Ny = Ny; self.template = template
        self.is_aqh = is_aqh; self.delta = delta_dialog

        lay = QVBoxLayout(self)
        self.fig = Figure(figsize=(5.8, 5.8)); self.fig.patch.set_facecolor(DARK_BG)
        self.canvas = FigureCanvas(self.fig)
        lay.addWidget(self.canvas, 1)
        self.ax = self.fig.add_subplot(111)
        row = QHBoxLayout()
        row.addWidget(QLabel('round trip'))
        self.sld = QSlider(Qt.Horizontal); self.sld.setRange(0, self.nrt - 1)
        self.sld.valueChanged.connect(self._on_slider)
        row.addWidget(self.sld, 1)
        self.lbl = QLabel(f'1 / {self.nrt}')
        row.addWidget(self.lbl)
        lay.addLayout(row)
        self.btn_movie = QPushButton('Save movie (MP4 / GIF)…')
        self.btn_movie.clicked.connect(self._save_movie)
        lay.addWidget(self.btn_movie)
        self._draw(0)

    def _draw(self, rt):
        ax = self.ax; ax.clear()
        title = f'δ = {self.delta:+.4f}   round trip {rt + 1}/{self.nrt}'
        if self.rep_grids is None:
            ax.axis('off')
            ax.text(0.5, 0.5, 'Ring movie not available for this run.\n'
                    'See the 2D map in the main window.',
                    transform=ax.transAxes, ha='center', va='center',
                    color=TEXT_DIM, fontsize=10)
        else:
            wf_by_grid = {g: self.wfr[rt, j] for j, g in enumerate(self.rep_grids)}
            if self.is_aqh:
                plot_waveform_distribution_aqh(ax, wf_by_grid, self.Nx, self.Ny,
                                               self.template, title=title, I_max=self.Imax)
            else:
                plot_waveform_distribution(ax, wf_by_grid, self.Nx, self.Ny,
                                           self.template, title=title, I_max=self.Imax)
        self.canvas.draw_idle()

    def _on_slider(self, v):
        self.lbl.setText(f'{v + 1} / {self.nrt}')
        self._draw(v)
    ...
    def _save_movie(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, 'Save slow-time movie', 'slowtime.mp4',
            'MP4 video (*.mp4);;GIF animation (*.gif)')
        if not path:
            return
        nframes = min(self.nrt, 300)
        idx = np.linspace(0, self.nrt - 1, nframes).astype(int)
        self.btn_movie.setEnabled(False); self.btn_movie.setText('Saving…')
        QApplication.processEvents()

        def upd(k):
            self._draw(int(idx[k])); return []
        anim = FuncAnimation(self.fig, upd, frames=nframes, blit=False)
        try:
            if path.lower().endswith('.gif'):
                anim.save(path, writer=PillowWriter(fps=20))
            else:
                from matplotlib.animation import FFMpegWriter
                anim.save(path, writer=FFMpegWriter(fps=20, bitrate=2400))
            self.btn_movie.setText(f'Saved: {os.path.basename(path)}')
        except Exception:
            try:                                   # fall back to GIF if ffmpeg is missing
                gif = os.path.splitext(path)[0] + '.gif'
                anim.save(gif, writer=PillowWriter(fps=20))
                self.btn_movie.setText(f'Saved GIF: {os.path.basename(gif)}')
            except Exception as e2:
                self.btn_movie.setText(f'Save failed: {e2}')
        finally:
            self.btn_movie.setEnabled(True)


class NonlinearSweepWorker(QThread):
    """Run lattice_kerr_sweep off the GUI thread. Emits one `step` per detuning with
    the display data (comb spectrum, pump line, soliton metric, settled field), so the
    main thread only does the plotting/storage. Numpy releases the GIL during the FFTs
    and matmuls, so the window stays responsive."""
    step = pyqtSignal(object)     # dict(i, comb, pump, spec, npk, ratio, field)
    done = pyqtSignal(bool)       # True = completed, False = stopped
    failed = pyqtSignal(str)
    ...
    def run(self):
        kw = self.kw
        try:
            def on_step(i, delta, comb, sd, E):
                pump_p = float(np.abs(sd[0]) ** 2)
                spec = np.fft.fftshift(np.abs(sd) ** 2)
                it = np.abs(np.fft.ifft(sd)) ** 2
                pk = float(it.max()); bg = float(np.median(it))
                above = it > 0.4 * pk
                npk = int(np.sum(np.diff(above.astype(int)) == 1))
                self.step.emit(dict(i=int(i), comb=float(comb), pump=pump_p, spec=spec,
                                    npk=npk, ratio=pk / max(bg, 1e-9),
                                    field=E.astype(np.complex64)))
                if self._stop:
                    raise _SweepStopped()
            lattice_kerr_sweep(
                kw['deltas_th'], kw['template'], kappa_ex=kw['kappa_ex'],
                alpha=kw['alpha'], beta0_eta_over_pi=kw['beta'], D2=kw['D2'],
                gamma=kw['gamma'], pump_amp=kw['pump_amp'], n_modes=kw['n_modes'],
                n_roundtrips=kw['n_roundtrips'], n_site_pts=kw['n_site_pts'],
                s_disp=kw['s_disp'], kerr_lumped=kw.get('kerr_lumped', False),
                on_step=on_step)
            self.done.emit(True)
...
class NonlinearDialog(QDialog):
    """Setup window for the nonlinear (Kerr) workflow.

    Shows the linear drop spectrum exactly as on the main window, but
    interactive: zoom / pan / home via the matplotlib toolbar, plus a yellow
    SpanSelector to pick an x-axis range (the pump-detuning scan window). A
    parameter panel exposes the nonlinear knobs (number of modes, pump power,
    Kerr gamma, dispersion, pump-detuning start / end / step).
    """
    def __init__(self, parent, omegas, Td):
        super().__init__(parent)
        self.setWindowTitle('Nonlinear — Kerr comb setup')
        self.resize(1340, 800)
        self.setStyleSheet(parent.styleSheet())   # inherit dark theme
        self.omegas = omegas
        self.Td = Td
        self.x = omegas / (2 * np.pi)
        self._sync = False        # guard against span <-> field feedback loops
        self._running = False     # True while a sweep is running
        self._stop = False        # (legacy) unused now that the sweep is threaded
        self._worker = None       # background sweep thread
        self._sel_idx = None      # selected detuning index (for slow-time analysis)
        self._fields = None       # settled field per detuning (stored during a sweep)
        self._st_ctx = None       # cavity/lattice context captured at sweep time
        self.btn_slow = None
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        # ── left: interactive spectrum + navigation toolbar ──────────────────
        left = QVBoxLayout()
        self.fig = Figure(figsize=(8.6, 6.2)); self.fig.patch.set_facecolor(DARK_BG)
        self.canvas = FigureCanvas(self.fig)
        gs = self.fig.add_gridspec(2, 2, height_ratios=[1.0, 1.25],
                                    left=0.13, right=0.90, top=0.94, bottom=0.09,
                                    hspace=0.45, wspace=0.55)
        self.ax = self.fig.add_subplot(gs[0, :])     # drop spectrum (pick range)
        self.ax2 = self.fig.add_subplot(gs[1, 0])    # comb power vs detuning (live)
        self.ax2pump = self.ax2.twinx()              # pump power (right y-axis)
        self.ax3 = self.fig.add_subplot(gs[1, 1])    # comb spectrum at a detuning
        self._spectra = []          # (delta, spectrum) recorded during the sweep
        self._mu = None
        self._sel_marker = None
        self._draw_spectrum()
        self._init_comb_axis()
        self._init_spec_axis()
        self.canvas.mpl_connect('button_press_event', self._on_pick)
        self.toolbar = NavigationToolbar2QT(self.canvas, self)
        # matplotlib's toolbar icons are dark glyphs — give the toolbar a light
        # background so they're visible against the app's dark theme.
        self.toolbar.setStyleSheet(
            'QToolBar{background:#c8d0e7;border:0;padding:2px;spacing:3px;}'
            'QToolButton{background:transparent;border-radius:3px;padding:2px;}'
            'QToolButton:hover{background:#9aa3c0;}'
            'QToolButton:checked{background:#8a93b5;}'
            'QLabel{color:#1a1f2e;background:transparent;}')
        left.addWidget(self.toolbar)
        left.addWidget(self.canvas, 1)
        root.addLayout(left, 1)

        xlo, xhi = float(self.x.min()), float(self.x.max())
        # default sweep: start blue at +0.02 (CW, above the band) and tune DOWN to
        # red -0.075 (high -> low) into the Kerr-tilted comb / soliton region.
        det_start = 0.057     # Detuning start = sweep start (high / blue)
        det_end = 0.036       # Detuning end   = sweep end   (low / red)

        # yellow x-range selector  ->  pump-detuning start / end
        self.span = SpanSelector(
            self.ax, self._on_span, 'horizontal', useblit=True,
            props=dict(alpha=0.30, facecolor='#ffe000'),
            interactive=True, drag_from_anywhere=True)

        # ── right: nonlinear parameter table ─────────────────────────────────
        box = QGroupBox('Nonlinear parameters'); box.setFixedWidth(300)
        form = QFormLayout(box)
        self.spn_modes = QSpinBox(); self.spn_modes.setRange(16, 8192)
        self.spn_modes.setSingleStep(16); self.spn_modes.setValue(256)
        self.spn_power = QDoubleSpinBox(); self.spn_power.setRange(0.0, 1e6)
        self.spn_power.setDecimals(4); self.spn_power.setSingleStep(0.005)
        self.spn_power.setValue(0.02)           # |psi_in|^2 (Ikeda pump power)
        self.spn_gamma = QDoubleSpinBox(); self.spn_gamma.setRange(0.0, 1e6)
        self.spn_gamma.setDecimals(5); self.spn_gamma.setSingleStep(0.01)
        self.spn_gamma.setValue(1.0)
        self.spn_d2 = QDoubleSpinBox(); self.spn_d2.setRange(-1e6, 1e6)
        self.spn_d2.setDecimals(5); self.spn_d2.setSingleStep(0.01)
        self.spn_d2.setValue(0.02)              # anomalous → solitons
        self.spn_nsteps = QSpinBox(); self.spn_nsteps.setRange(10, 200000)
        self.spn_nsteps.setSingleStep(100); self.spn_nsteps.setValue(400)
        self.spn_det0 = QDoubleSpinBox(); self.spn_det0.setRange(-1e3, 1e3)
        self.spn_det0.setDecimals(5); self.spn_det0.setSingleStep(0.001)
        self.spn_det1 = QDoubleSpinBox(); self.spn_det1.setRange(-1e3, 1e3)
        self.spn_det1.setDecimals(5); self.spn_det1.setSingleStep(0.001)
        self.spn_dets = QDoubleSpinBox(); self.spn_dets.setRange(1e-6, 1e3)
        self.spn_dets.setDecimals(6); self.spn_dets.setSingleStep(0.00005)
        self.spn_dets.setValue(0.0002)
        self.chk_lumped = QCheckBox('Lumped Kerr (1 kick / round trip, faster)')
        self.chk_lumped.setChecked(False)   # default: Strang split-step
        self.chk_lumped.setToolTip(
            'Unchecked (default): second-order Strang split-step — the Kerr phase is\n'
            'integrated continuously along the round trip (Nz kicks). Most accurate.\n\n'
            'Checked: lumped Kerr — a single Kerr kick per round trip. Nz× fewer FFTs,\n'
            'so much faster, but only first-order accurate in the step size.')
        form.addRow('Number of modes', self.spn_modes)
        form.addRow('Pump power', self.spn_power)
        form.addRow('Kerr γ', self.spn_gamma)
        form.addRow('Dispersion D₂', self.spn_d2)
        form.addRow('Detuning start  ω/(2π)', self.spn_det0)
        form.addRow('Detuning end  ω/(2π)', self.spn_det1)
        form.addRow('Detuning step  ω/(2π)', self.spn_dets)
        form.addRow('Round trips / detuning', self.spn_nsteps)
        form.addRow('Kerr integration', self.chk_lumped)
        for s in (self.spn_det0, self.spn_det1):
            s.valueChanged.connect(self._on_fields)

        self.btn_run = QPushButton('Run nonlinear')
        self.btn_run.clicked.connect(self._run)
        self.btn_close = QPushButton('Close')
        self.btn_close.clicked.connect(self._on_close_or_stop)
        self.btn_save = QPushButton('Save run…')
        self.btn_save.clicked.connect(self._save_state)
        self.btn_load = QPushButton('Load run…')
        self.btn_load.clicked.connect(self._load_state)
        self.progress = QProgressBar()
        self.progress.setRange(0, 100); self.progress.setValue(0)
        self.progress.setFixedHeight(16); self.progress.setTextVisible(True)
        self.lbl_info = QLabel('Drag a yellow range on the spectrum to set the '
                                'pump-detuning scan window (or type start/end).')
        self.lbl_info.setWordWrap(True)
        self.lbl_info.setStyleSheet(f'color:{TEXT_DIM};font-size:10px;')

        # slow-time analysis: evolve the selected detuning's settled field
        st_box = QGroupBox('Slow-time analysis'); st_box.setFixedWidth(300)
        st_form = QFormLayout(st_box)
        self.spn_slowrt = QSpinBox(); self.spn_slowrt.setRange(10, 200000)
        self.spn_slowrt.setSingleStep(100); self.spn_slowrt.setValue(500)
        st_form.addRow('Round trips', self.spn_slowrt)
        self.btn_slow = QPushButton('Slow-time analysis')
        self.btn_slow.setEnabled(False)
        self.btn_slow.clicked.connect(self._slow_time)
        st_form.addRow(self.btn_slow)
        self.lbl_slow = QLabel('Click a detuning on the sweep, then run.')
        self.lbl_slow.setWordWrap(True)
        self.lbl_slow.setStyleSheet(f'color:{TEXT_DIM};font-size:10px;')
        st_form.addRow(self.lbl_slow)
        # 2D slow-time map (fast time x round trip), filled after a run
        self.st_map_fig = Figure(figsize=(2.9, 2.0)); self.st_map_fig.patch.set_facecolor(DARK_BG)
        self.st_map_canvas = FigureCanvas(self.st_map_fig)
        self.st_map_canvas.setMinimumHeight(190)
        st_form.addRow(self.st_map_canvas)
        self.btn_save_stmap = QPushButton('Save map (PNG + SVG)')
        self.btn_save_stmap.setEnabled(False)
        self.btn_save_stmap.clicked.connect(self._save_st_map)
        st_form.addRow(self.btn_save_stmap)
        ...
        sl_row = QHBoxLayout()
        sl_row.addWidget(self.btn_save); sl_row.addWidget(self.btn_load)
        right.addLayout(sl_row)
        right.addWidget(self.progress)
        right.addWidget(self.lbl_info)
        right.addWidget(st_box)
        right.addStretch(1)
        root.addLayout(right, 0)

        # initialise the fields (start = high/blue, end = low/red) and yellow band.
        # The sweep always runs high -> low, so the fields need not be ordered.
        self._sync = True
        self.spn_det0.setValue(det_start)
        self.spn_det1.setValue(det_end)
        lo = min(max(min(det_start, det_end), xlo), xhi)   # clamp visual band to the plot
        hi = min(max(max(det_start, det_end), xlo), xhi)
        try:
    ...
    def _draw_spectrum(self):
        ...
        self.ax.plot(self.x, self.Td, lw=1.0, color='#ff4a6e')
        self.ax.set_ylim(0.0, max(float(self.Td.max()) * 1.10, 1e-6))
        self.ax.set_xlim(float(self.x.min()), float(self.x.max()))
        self.ax.set_xlabel('ω/(2π)  (FSR units)', color=TEXT_COL, fontsize=9)
        self.ax.set_ylabel('T_drop', color=TEXT_COL, fontsize=9)
        self.ax.set_title('Drop spectrum', color=ACCENT, fontsize=10)
        self.ax.tick_params(colors=TEXT_COL, labelsize=8)
        for sp in self.ax.spines.values():
            sp.set_color(SPINE_COL)
        self.ax.grid(True, color=GRID_COL, alpha=0.5)
        _ends_ticks(self.ax)
        self.canvas.draw_idle()

    def _init_comb_axis(self):
        ax = self.ax2; ax.clear(); self.ax2pump.clear()
        ax.set_facecolor(PANEL_BG)
        ax.set_xlim(float(self.x.min()), float(self.x.max()))
        ax.set_xlabel('pump detuning  ω/(2π)', color=TEXT_COL, fontsize=9)
        ax.set_ylabel('comb power (no pump)', color='#ffd000', fontsize=9)
        ax.set_title('Detuning sweep — click a point', color=ACCENT, fontsize=10)
        ax.tick_params(axis='x', colors=TEXT_COL, labelsize=8)
        ax.tick_params(axis='y', colors='#ffd000', labelsize=8)
        for sp in ax.spines.values():
            sp.set_color(SPINE_COL)
        ax.grid(True, color=GRID_COL, alpha=0.5)
        # right y-axis: pump (mu = 0) power
        self.ax2pump.yaxis.set_label_position('right')
        self.ax2pump.yaxis.set_ticks_position('right')
        self.ax2pump.set_ylabel('pump power', color='#4a9eff', fontsize=9)
        self.ax2pump.tick_params(axis='y', colors='#4a9eff', labelsize=8)
        for sp in self.ax2pump.spines.values():
            sp.set_color(SPINE_COL)
        ax.margins(y=0); self.ax2pump.margins(y=0)   # no padding -> ticks at the data extremes
        _ends_ticks(ax)                       # min/max only on x and left y
        _ends_ticks(self.ax2pump, xaxis=False)  # min/max only on right y
        self._sel_marker = None
        self.canvas.draw_idle()

    def _init_spec_axis(self):
        ax = self.ax3; ax.clear()
        ax.set_facecolor(PANEL_BG)
        ax.set_xlabel('mode μ (FSR units)', color=TEXT_COL, fontsize=9)
        ax.set_ylabel('comb power (dB, norm.)', color=TEXT_COL, fontsize=9)
        ax.set_title('Comb at selected detuning', color=ACCENT, fontsize=10)
        ax.tick_params(colors=TEXT_COL, labelsize=8)
        for sp in ax.spines.values():
            sp.set_color(SPINE_COL)
        ax.grid(True, color=GRID_COL, alpha=0.4, which='both')
        ax.text(0.5, 0.5, 'run a sweep, then click the\nleft plot to show a comb',
                transform=ax.transAxes, ha='center', va='center',
    ...
    def _on_pick(self, event):
        """Click on the sweep plot -> show the comb spectrum at that detuning.
        Accept clicks on the comb axis OR its pump-power twin (the twin overlays
        the comb axis, so clicks usually land on it)."""
        if (event.inaxes not in (self.ax2, self.ax2pump)
                or not self._spectra or event.xdata is None):
            return
        deltas = np.array([d for d, _ in self._spectra])
        i = int(np.argmin(np.abs(deltas - event.xdata)))
        self._show_comb(i)

    def _show_comb(self, i):
        self._sel_idx = i                      # remember selection for slow-time analysis
        if getattr(self, 'btn_slow', None) is not None:
            self.btn_slow.setEnabled(self._fields is not None and i < len(self._fields))
        delta, spec = self._spectra[i]
        N = len(spec)
        if self._mu is None or len(self._mu) != N:
            self._mu = np.fft.fftshift(np.fft.fftfreq(N, d=1.0 / N))
        ax = self.ax3; ax.clear()
        ax.set_facecolor(PANEL_BG)
        sp_n = spec / (spec.max() if spec.max() > 0 else 1.0)
        sp_db = 10.0 * np.log10(np.clip(sp_n, 1e-8, None))   # dB, peak = 0 dB
        ax.plot(self._mu, sp_db, lw=0.8, color='#ff4a6e')
        ax.set_xlim(-N / 2, N / 2); ax.set_ylim(-80.0, 3.0)
        ax.set_xlabel('mode μ (FSR units)', color=TEXT_COL, fontsize=9)
        ax.set_ylabel('comb power (dB, norm.)', color=TEXT_COL, fontsize=9)
        ax.set_title(f'Comb at δ = {delta:+.4f}', color=ACCENT, fontsize=10)
        ax.tick_params(colors=TEXT_COL, labelsize=8)
        for sp in ax.spines.values():
            sp.set_color(SPINE_COL)
        ax.grid(True, color=GRID_COL, alpha=0.4, which='both')
        if self._sel_marker is not None:
            try:
                ...
                pass
        self._sel_marker = self.ax2.axvline(delta, color='#ffffff', lw=0.9,
                                             ls=':', alpha=0.85)
        _ends_ticks(ax)
        self.canvas.draw_idle()

    def _set_span(self, a, b):
        lo, hi = min(a, b), max(a, b)
        self._sync = True
        self.spn_det0.setValue(lo); self.spn_det1.setValue(hi)
        try:
            self.span.extents = (lo, hi)
        except Exception:
            pass
    ...
    def _on_span(self, xmin, xmax):
        """User dragged the yellow range -> update the detuning fields. Start is the
        HIGH edge, end is the LOW edge (the sweep runs high -> low). Ignore a
        zero-width selection (a plain click) so it can't collapse the range."""
        if self._sync or abs(xmax - xmin) < 1e-6:
            return
        self._sync = True
        self.spn_det0.setValue(xmax)   # Detuning start = high (blue)
        self.spn_det1.setValue(xmin)   # Detuning end   = low (red)
        self._sync = False

    def _on_fields(self, *_):
        """User typed start/end -> move the yellow range to match."""
        if self._sync:
            return
        a, b = self.spn_det0.value(), self.spn_det1.value()
        self._sync = True
        try:
            self.span.extents = (min(a, b), max(a, b))
        except Exception:
            pass
        self._sync = False
        self.canvas.draw_idle()

    def _eta_text(self, done, total, t0):
        """Remaining time + estimated finish clock, from the rate so far."""
        if done <= 0:
            return ''
        rem = (time.time() - t0) / done * (total - done)
        fin = time.strftime('%H:%M:%S', time.localtime(time.time() + rem))
        s = int(rem); h, r = divmod(s, 3600); mm, ss = divmod(r, 60)
        rs = f'{h}h{mm:02d}m' if h else (f'{mm}m{ss:02d}s' if mm else f'{ss}s')
        return f'ETA {rs} · finish ~{fin}'

    def _on_close_or_stop(self):
        """The bottom button is Stop while a sweep runs (abort it), Close otherwise."""
        if self._running and getattr(self, '_worker', None) is not None:
            self._worker._stop = True
            self.btn_close.setText('Stopping…')
        else:
            self.close()

    def closeEvent(self, event):
        """Stop and wait for the sweep thread before the window is destroyed."""
    ...
    def _slow_time(self):
        """Take the selected detuning's stored settled field, hold delta fixed, and
        evolve for N round trips -> show |E(tau)|^2 vs round trip and the ring movie."""
        if (self._fields is None or self._sel_idx is None or self._st_ctx is None
                or self._sel_idx >= len(self._fields)):
            self.lbl_slow.setText('Click a detuning on the sweep first.')
            return
        i = self._sel_idx; ctx = self._st_ctx
        E0 = np.asarray(self._fields[i], dtype=complex)
        delta = float(ctx['deltas_th'][i])
        d_dialog = float(self._spectra[i][0])
        nrt = int(self.spn_slowrt.value())
        self.btn_slow.setEnabled(False); self.btn_slow.setText('Evolving…')
        self.progress.setRange(0, nrt); self.progress.setValue(0)
        st_t0 = time.time()

        def on_rt(rt):
            if rt % 10 == 0 or rt == nrt - 1:
                self.progress.setValue(rt + 1)
                self.lbl_slow.setText(
                    f'evolving… {rt + 1}/{nrt}   {self._eta_text(rt + 1, nrt, st_t0)}')
                QApplication.processEvents()
        try:
            wf, wfr = slow_time_evolve(
                E0, ctx['template'], ctx['kappa_ex'], ctx['alpha'], ctx['beta'],
                ctx['D2'], ctx['gamma'], ctx['pump_amp'], delta, nrt,
                s_disp=ctx['s_disp'], kerr_lumped=ctx.get('kerr_lumped', False),
                rep_grids=ctx['rep_grids'], on_rt=on_rt)
            self._st_last = dict(wf=wf, wfr=wfr, delta=d_dialog)   # for save
            self._draw_st_map(wf, d_dialog)                # 2D fast-time map in the dialog
            self.lbl_slow.setText(
                f'Done — {nrt} round trips at δ = {d_dialog:+.4f}.')
            SlowTimeDialog(self, wfr, ctx['rep_grids'], ctx['Nx'], ctx['Ny'],
                           ctx['template'], ctx['is_aqh'], d_dialog).exec_()  # ring movie
        finally:
            self.btn_slow.setEnabled(True); self.btn_slow.setText('Slow-time analysis')

    def _draw_st_map(self, hist, d_dialog):
        """Draw the 2D slow-time map (fast time x round trip) in the dialog panel."""
        self.st_map_fig.clear()
        ax = self.st_map_fig.add_subplot(111); ax.set_facecolor(PANEL_BG)
        nrt, N = hist.shape
        ax.imshow(hist, origin='lower', aspect='auto', cmap='viridis',
                  extent=[-N / 2, N / 2, 0, nrt], interpolation='nearest')
        ax.set_xlabel('τ', color=TEXT_COL, fontsize=7)
        ax.set_ylabel('round trip', color=TEXT_COL, fontsize=7)
        ax.set_xticks([-N / 2, 0.0, N / 2])          # min, center, max
        ax.set_yticks([0, nrt])                       # min, max only
        ax.tick_params(colors=TEXT_COL, labelsize=6)
        for sp in ax.spines.values():
            sp.set_color(SPINE_COL)
        self.st_map_fig.tight_layout()
        self.st_map_canvas.draw_idle()
        if hasattr(self, 'btn_save_stmap'):
            self.btn_save_stmap.setEnabled(True)

    def _save_st_map(self):
        """Save the slow-time 2D map to disk in both PNG and SVG."""
        path, _ = QFileDialog.getSaveFileName(
            self, 'Save slow-time map', 'slow_time_map.png',
            'Images (*.png *.svg)')
        if not path:
            return
        ...
        try:
            for ext in ('png', 'svg'):
                self.st_map_fig.savefig(f'{base}.{ext}', dpi=200,
                                        facecolor=self.st_map_fig.get_facecolor())
            self.lbl_slow.setText(
                f'Saved map to {os.path.basename(base)}.png / .svg')
        except Exception as e:
            self.lbl_slow.setText(f'Save failed: {e!r}')

    # ── save / load a whole nonlinear run ────────────────────────────────────
    def _save_state(self):
        """Save the whole run to a .npz: params, sweep curves, comb spectra, the
        per-detuning settled fields, the drop spectrum, and the last slow-time map."""
        from PyQt5.QtWidgets import QFileDialog
        if not self._spectra or self._st_ctx is None:
            self.lbl_info.setText('Nothing to save — run a sweep first.')
            return
        path, _ = QFileDialog.getSaveFileName(
            self, 'Save nonlinear run', 'nonlinear_run.npz', 'NumPy archive (*.npz)')
        if not path:
            return
        ctx = self._st_ctx
        meta = dict(build=ctx['build_params'], dialog=self.params(),
                    s_disp=float(ctx['s_disp']), n_site_pts=int(ctx['n_site_pts']),
                    n_modes=int(ctx['n_modes']),
                    rep_grids=(list(map(int, ctx['rep_grids']))
                               if ctx['rep_grids'] is not None else None))
        deltas = np.array([d for d, _ in self._spectra], dtype=float)
        d = dict(meta=np.array(json.dumps(meta)),
                 x=np.asarray(self.x, float), Td=np.asarray(self.Td, float),
                 deltas=deltas,
                 comb_power=np.array(self._ys, float),
                 pump_power=np.array(self._pp, float),
                 spectra=np.array([s for _, s in self._spectra], float),
                 solscore=np.array(self._solscore, float),
                 fields=np.array(self._fields, dtype=np.complex64))
        if self._st_last is not None:
            d['st_wf'] = self._st_last['wf']; d['st_wfr'] = self._st_last['wfr']
            d['st_delta'] = np.array(float(self._st_last['delta']))
        try:
            self.lbl_info.setText('Saving…'); QApplication.processEvents()
            np.savez_compressed(path, **d)
            integ = 'lumped Kerr' if self.chk_lumped.isChecked() else 'Strang split-step'
            self.lbl_info.setText(
                f'Saved run to {os.path.basename(path)} — {integ}.')
        except Exception as e:
            self.lbl_info.setText(f'Save failed: {e}')

    def _load_state(self):
        """Load a saved run and replicate every plot + the slow-time data."""
        from PyQt5.QtWidgets import QFileDialog
        path, _ = QFileDialog.getOpenFileName(
            self, 'Load nonlinear run', '', 'NumPy archive (*.npz)')
        if not path:
            return
        try:
            z = np.load(path, allow_pickle=True)
            meta = json.loads(str(z['meta'])); bp = meta['build']; dp = meta['dialog']
        except Exception as e:
            self.lbl_info.setText(f'Load failed: {e}')
            return
        # rebuild the exact template from the build parameters
        if bp['lat'] == 'AQH':
            is_aqh = True
            bi, bd = aqh_default_bus_positions(bp['Nx'], bp['Ny'])
            template = build_template_aqh(bp['Nx'], bp['Ny'], bp['phi'], bp['kappa_ex'],
                                          bp['kappa_J'], bus_in_idx=bi, bus_drop_idx=bd,
                                          Nz_site=bp['Nz_site'], Nz_link=bp['Nz_link'])
        else:
            is_aqh = False
            bi, bd = default_bus_positions(bp['Nx'], bp['Ny'], Nz_site=bp['Nz_site'])
            template = build_template(bp['Nx'], bp['Ny'], bp['phi'], bp['kappa_ex'],
                                      bp['kappa_J'], bus_in=bi, bus_drop=bd,
                                      Nz_site=bp['Nz_site'], Nz_link=bp['Nz_link'])
        # drop spectrum
        self.x = np.asarray(z['x']); self.Td = np.asarray(z['Td'])
        self.omegas = self.x * (2 * np.pi)
        self._draw_spectrum()
        # sweep data
        deltas = z['deltas']; spectra = z['spectra']
        self._spectra = [(float(deltas[i]), spectra[i]) for i in range(len(deltas))]
        self._fields = [np.asarray(f) for f in z['fields']]
        self._solscore = [tuple(r) for r in z['solscore']]
        self._xs = list(map(float, deltas))
        self._ys = list(map(float, z['comb_power']))
        self._pp = list(map(float, z['pump_power']))
        N = int(meta['n_modes'])
        self._mu = np.fft.fftshift(np.fft.fftfreq(N, d=1.0 / N))
        self._sel_idx = None
        self._st_ctx = dict(template=template, kappa_ex=bp['kappa_ex'], alpha=bp['alpha'],
                            beta=bp['beta0_eta'], D2=abs(float(dp['D2'])),
                            gamma=float(dp['gamma']),
                            pump_amp=np.sqrt(max(float(dp['pump_power']), 0.0)),
                            s_disp=float(meta['s_disp']),
                            deltas_th=2 * np.pi * np.asarray(deltas, float),
                            Nx=bp['Nx'], Ny=bp['Ny'], is_aqh=is_aqh,
                            rep_grids=meta['rep_grids'], build_params=bp,
                            n_site_pts=int(meta['n_site_pts']), n_modes=N,
                            kerr_lumped=bool(dp.get('kerr_lumped', False)))
        # restore the dialog fields
        self._sync = True
        self.spn_modes.setValue(int(dp['n_modes'])); self.spn_power.setValue(dp['pump_power'])
        self.spn_gamma.setValue(dp['gamma']); self.spn_d2.setValue(dp['D2'])
        self.spn_det0.setValue(dp['det_start']); self.spn_det1.setValue(dp['det_end'])
        self.spn_dets.setValue(dp['det_step']); self.spn_nsteps.setValue(int(dp['n_steps']))
        self.chk_lumped.setChecked(bool(dp.get('kerr_lumped', False)))
        self._sync = False
        # replot the sweep + slow-time map
        self._replot_sweep()
        self._st_last = None
        if 'st_wf' in z.files:
            self._st_last = dict(wf=z['st_wf'], wfr=z['st_wfr'], delta=float(z['st_delta']))
            self._draw_st_map(z['st_wf'], float(z['st_delta']))
        integ = 'lumped Kerr' if dp.get('kerr_lumped', False) else 'Strang split-step'
        self.lbl_info.setText(
            f'Loaded {os.path.basename(path)} — {len(deltas)} detunings '
            f'({bp["Nx"]}x{bp["Ny"]} {bp["lat"]}), {integ}.')

    def _replot_sweep(self):
        """Redraw the detuning-sweep panel from stored curves and auto-show a comb."""
        self._init_comb_axis(); self._init_spec_axis()
        if not self._xs:
            return
        self.ax2.set_xlim(min(self._xs), max(self._xs))
        self.ax2.plot(self._xs, self._ys, lw=1.2, color='#ffd000')
        self.ax2pump.plot(self._xs, self._pp, lw=1.0, color='#4a9eff', alpha=0.9)
        self.ax2.relim(); self.ax2.autoscale_view(scalex=False, scaley=True)
        self.ax2pump.relim(); self.ax2pump.autoscale_view(scalex=False, scaley=True)
        self.canvas.draw_idle()
        if self._solscore:
            single = [(j, r) for j, (n, r) in enumerate(self._solscore) if n == 1]
            j = (max(single, key=lambda t: t[1])[0] if single
                 else int(np.argmax(self._ys)))
            self._show_comb(j)

    def params(self):
        """Return the chosen nonlinear parameters as a dict."""
        return dict(
            n_modes=self.spn_modes.value(),
            pump_power=self.spn_power.value(),
            gamma=self.spn_gamma.value(),
            D2=self.spn_d2.value(),
            det_start=self.spn_det0.value(),
            det_end=self.spn_det1.value(),
            det_step=self.spn_dets.value(),
            n_steps=self.spn_nsteps.value(),
            kerr_lumped=self.chk_lumped.isChecked(),
        )

    def _run(self):
        """Sweep the pump detuning across the chosen range (carrying the field
        forward), running the coupled-LLE / dispersive-Ikeda map at each step,
        and plot the comb power (pump mode excluded, summed over sites) vs
        detuning live — revealing the soliton step. Works for any lattice the
        linear app can build (single ring, IQH, AQH)."""
        p = self.params()
        # Drive the SAME z-resolved propagator the linear app built (site rings +
        # link rings, no effective Hamiltonian). Reuse the template from the last
        # linear sweep so the geometry/coupling are identical.
        par = self.parent()
        template = par.state.get('template') if hasattr(par, 'state') else None
        if template is None:
            self.lbl_info.setText('Run a linear sweep first (Compute), then Run nonlinear.')
            return
        try:
            kex = float(par.spn_kex.value()); alpha = float(par.spn_alpha.value())
            beta = float(par.spn_beta0eta.value())          # beta0_eta_over_pi
            Nx = int(par.spn_nx.value()); Ny = int(par.spn_ny.value())
            is_aqh = par._is_aqh()
        except Exception:
            kex, alpha, beta, Nx, Ny, is_aqh = 0.199, 0.01, 1.0, 1, 1, False
        lat = 'AQH' if is_aqh else 'IQH'
        try:
            kJ = float(par.spn_kJ.value()); phi = float(par.spn_phi.value()) * np.pi
        except Exception:
            kJ, phi = 0.5, 0.5 * np.pi
        Nz_site = template['Nz_site']; Nz_link = template['Nz_link']
        n_sites = len(aqh_site_positions(Nx, Ny)) if is_aqh else Nx * Ny
        n_site_pts = n_sites * Nz_site                       # site grid points (MI noise seed)
        build_params = dict(lat=lat, Nx=Nx, Ny=Ny, kappa_ex=kex, kappa_J=kJ,
                            phi=phi, beta0_eta=beta, alpha=alpha,
                            Nz_site=Nz_site, Nz_link=Nz_link)   # to rebuild the template on load

        d0, d1 = p['det_start'], p['det_end']
        ds = max(p['det_step'], 1e-12)
        npts = max(int(round(abs(d1 - d0) / ds)) + 1, 2)
        # Sweep blue -> red = HIGH detuning -> LOW (delta>0 = pump above the cold
        # resonance; Kerr tilts the resonance to negative delta, so solitons sit on
        # the upper branch reached by tuning down). Always descending, regardless of
        # how the range was entered.
        deltas = np.linspace(max(d0, d1), min(d0, d1), npts)
        deltas_th = 2.0 * np.pi * deltas                    # detuning carrier omega (= solve_one omega)
        D2 = float(p['D2'])
        s_disp = -1.0 if D2 >= 0 else 1.0                   # s = -1 anomalous (bright solitons)
        pump_amp = np.sqrt(max(p['pump_power'], 0.0))       # pump field E_in at the input bus

        # fresh axes: comb power (left, yellow) + pump power (right, blue)
        self._init_comb_axis(); self._init_spec_axis()
        self.ax2.set_xlim(min(d0, d1), max(d0, d1))
        (line,) = self.ax2.plot([], [], lw=1.2, color='#ffd000')
        (pline,) = self.ax2pump.plot([], [], lw=1.0, color='#4a9eff', alpha=0.9)
        xs, ys, pp = [], [], []
        self._xs, self._ys, self._pp = xs, ys, pp   # keep refs for save/replot
        self._st_last = None                         # last slow-time result (2D map + ring data)
        self._spectra = []
        self._fields = []            # settled complex field per detuning (for slow-time)
        self._solscore = []          # (npk, peak/bg) per detuning, for soliton auto-show
        self._sel_idx = None
        if not is_aqh:                              # one representative grid point per ring
            rep_grids = ([template["site_idx"](ix, iy, 0)
                          for iy in range(Ny) for ix in range(Nx)]
                         + [template["link_idx"](nm, 0)
                            for nm in template["h_link_names"] + template["v_link_names"]])
        else:                                       # AQH: slot 0 of each site & link ring
            n_aqh_sites = len(aqh_site_positions(Nx, Ny))
            n_aqh_links = len(aqh_link_positions(Nx, Ny))
            rep_grids = ([template["site_idx"](i, 0) for i in range(1, n_aqh_sites + 1)]
                         + [template["link_idx"](j, 0) for j in range(1, n_aqh_links + 1)])
        self._st_ctx = dict(template=template, kappa_ex=kex, alpha=alpha, beta=beta,
                            D2=abs(D2), gamma=p['gamma'], pump_amp=pump_amp,
                            s_disp=s_disp, deltas_th=deltas_th,
                            Nx=Nx, Ny=Ny, is_aqh=is_aqh, rep_grids=rep_grids,
                            build_params=build_params, n_site_pts=n_site_pts,
                            n_modes=int(p['n_modes']), kerr_lumped=p['kerr_lumped'])
        self._mu = np.fft.fftshift(np.fft.fftfreq(int(p['n_modes']),
                                                   d=1.0 / int(p['n_modes'])))

        # keep refs the signal handler needs (the sweep runs off-thread now)
        self._line = line; self._pline = pline; self._deltas = deltas
        self._npts = npts; self._n_sites = n_sites; self._lat = lat

        self.btn_run.setEnabled(False); self.btn_run.setText('Running…')
        self._running = True
        self.btn_close.setText('Stop')
        self.progress.setRange(0, npts); self.progress.setValue(0)
        self._sweep_t0 = time.time()

        kw = dict(deltas_th=deltas_th, template=template, kappa_ex=kex, alpha=alpha,
                  beta=beta, D2=abs(D2), gamma=p['gamma'], pump_amp=pump_amp,
                  n_modes=p['n_modes'], n_roundtrips=p['n_steps'],
                  n_site_pts=n_site_pts, s_disp=s_disp,
                  kerr_lumped=p['kerr_lumped'])
        self._worker = NonlinearSweepWorker(kw)
        self._worker.step.connect(self._on_sweep_step)
        self._worker.done.connect(self._on_sweep_done)
        self._worker.failed.connect(self._on_sweep_failed)
        self._worker.start()                        # -> returns immediately; GUI stays live

    def _on_sweep_step(self, d):
        """Main-thread handler: one detuning of results from the worker."""
        i = d['i']
        self._fields.append(d['field'])
        self._solscore.append((d['npk'], d['ratio']))
        d_dialog = float(self._deltas[i])
        self._spectra.append((d_dialog, d['spec']))
        self._xs.append(d_dialog); self._ys.append(d['comb']); self._pp.append(d['pump'])
        self._line.set_data(self._xs, self._ys)
        self._pline.set_data(self._xs, self._pp)
        self.ax2.relim(); self.ax2.autoscale_view(scalex=False, scaley=True)
        self.ax2pump.relim(); self.ax2pump.autoscale_view(scalex=False, scaley=True)
        self.progress.setValue(i + 1)
        self.lbl_info.setText(
            f'δ = {d_dialog:+.4f}   ({i + 1}/{self._npts})   comb = {d["comb"]:.4f}   '
            f'{self._eta_text(i + 1, self._npts, self._sweep_t0)}')
        if i % 2 == 0 or i == self._npts - 1:
            self.canvas.draw_idle()

    def _on_sweep_done(self, completed):
        if self._spectra:                           # auto-show a clean single soliton if any
            single = [(j, r) for j, (n, r) in enumerate(self._solscore) if n == 1]
            j = (max(single, key=lambda t: t[1])[0] if single
                 else int(np.argmax(self._ys)))
            self._show_comb(j)
        if completed:
            self.lbl_info.setText(
                f'Done — {self._npts} detunings, {self._n_sites} ring(s) ({self._lat}). '
                f'Click the left plot to inspect the comb at any detuning.')
        else:
            self.lbl_info.setText(
                f'Stopped at {len(self._spectra)}/{self._npts} detunings. Click the '
                f'left plot to inspect any comb collected so far.')
        self._finish_sweep()

    def _on_sweep_failed(self, msg):
        self.lbl_info.setText(f'Sweep failed: {msg}')
        self._finish_sweep()

    def _finish_sweep(self):
        self._running = False
        self.btn_close.setText('Close')
        self.btn_run.setEnabled(True); self.btn_run.setText('Run nonlinear')
        self.canvas.draw_idle()
        self._worker = None
...
class MainWindow(QMainWindow):
    ...
    def _build_controls(self):
        ...
        self.spn_kex.setSingleStep(0.005); self.spn_kex.setValue(0.200)
        ...
        self.spn_kJ.setSingleStep(0.005); self.spn_kJ.setValue(0.750)
        ...
        self.spn_omin.setDecimals(3); self.spn_omin.setValue(-0.2)
        self.spn_omax = QDoubleSpinBox()
        self.spn_omax.setRange(-10.0, 10.0); self.spn_omax.setSingleStep(0.01)
        self.spn_omax.setDecimals(3); self.spn_omax.setValue(0.2)
        ...
        # Nonlinear: greyed out until a linear sweep has completed.
        self.btn_nonlinear = QPushButton('Nonlinear')
        self.btn_nonlinear.setFixedWidth(90); self.btn_nonlinear.setFixedHeight(28)
        self.btn_nonlinear.setEnabled(False)
        self.btn_nonlinear.clicked.connect(self._open_nonlinear)
        self.btn_nonlinear.setStyleSheet(
            'QPushButton:enabled{color:#00e5ff;border-color:#00e5ff;}'
            'QPushButton:disabled{color:#2a3050;}')
        self.btn_nonlinear.setToolTip(
            'Available after a linear sweep completes (▶ Compute).')
        ...
        # row 4: save path (shortened to make room for the Nonlinear button)
        rl.addWidget(self.lbl_save_path,  4, 0)
        rl.addWidget(self.edit_save_path, 4, 1, 1, 3)
        rl.addWidget(self.btn_browse,     4, 4)
        rl.addWidget(self.btn_nonlinear,  4, 5)
    ...
    def _invalidate(self):
        ...
        self.btn_save.setEnabled(False)
        self.btn_nonlinear.setEnabled(False)  # no current completed sweep
        self.cmb_peak.setEnabled(False)
    ...
    def _on_scan_done(self, omegas, Td, Tt, template):
        ...
        self.btn_nonlinear.setEnabled(True)  # linear sweep complete
        self.btn_run.setEnabled(True); self.btn_run.setText('▶ Compute')
        self._set_params_enabled(True)
    ...
    def _reset_all(self):
        ...
        self.spn_kex.setValue(0.200)      # bus↔site coupling
        self.spn_kJ.setValue(0.750)       # site↔link coupling
        self.spn_beta0eta.setValue(1.0)   # full anti-resonance
        self.spn_alpha.setValue(0.01)     # ~1.2 GHz intrinsic linewidth
        self.spn_omin.setValue(-0.2)
        self.spn_omax.setValue(0.2)
        self.spn_npts.setValue(4001)
        self.state['removed_sites'] = set()
        self.state['removed_links'] = set()
        self._invalidate()
    ...
    def _open_nonlinear(self):
        """Open the nonlinear setup window (enabled once a linear sweep has
        completed): the interactive drop spectrum + nonlinear parameter table."""
        omegas = self.state.get('omegas')
        Td = self.state.get('Td')
        if omegas is None or Td is None:
            QMessageBox.information(
                self, 'No spectrum yet',
                'Run a linear sweep first (▶ Compute), then open Nonlinear.')
            return
        dlg = NonlinearDialog(self, omegas, Td)
        self._nl_dialog = dlg   # keep a reference so it isn't garbage-collected
        dlg.show()
